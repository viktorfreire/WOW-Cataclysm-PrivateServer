...
// $Id: calendar.js,v 1.11 2005/04/07 00:56:11 dror Exp $
...
Zapatec.Calendar.dayMouseDown = function(ev) {
	...
	if (el.navtype != 300) {
		if (el.navtype == 50) {
			...
			if (((el.navtype == 201) || (el.navtype == 202)) && (cal.timeInterval > 30) && (cel.className.indexOf("minute", 0) != -1)) {canDrag = false;}
...
Zapatec.Calendar.dayMouseOver = function(ev) {
	...
	if (el.navtype != 300) {
		if (!((cal.timeInterval === null) || ((cal.timeInterval < 60) && (el.className.indexOf("hour", 0) != -1))) && (el.navtype == 50)) {return Zapatec.Utils.stopEvent(ev);}
			...
			//ALLOWED TIME CHECK
				...
				if (this.timeInterval) {
					if (this.timeInterval > 30) {
						mins = 0;
						hrs = (hrs % (this.timeInterval / 60) == 0) ? (hrs) : (hrs - (hrs % (this.timeInterval / 60)) + this.timeInterval / 60);
					} else {
						mins = (mins % this.timeInterval == 0) ? (mins) : (mins - (mins % this.timeInterval) + this.timeInterval);
					}
				}
			//END OF ALLOWED TIME CHECK
				var pm = (hrs > 12);
